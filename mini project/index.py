...
class user:
    ...
    def login(self):
        ...
        if result:
            ...
            self.head['text'] = "Welcome S92069531 " + self.username.get()
...
class travel:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Sl Bloom Cab Services system")
        ...
        Number_cars = StringVar()
        Number_van = StringVar()
        Number_threewheel = StringVar()
        Number_Trucks = StringVar()
        Number_Lorry = StringVar()
        ...
        def Receiptt():
            if reset_counter == 0 and Firstname.get() != "" and Surname.get() != "" and Address.get() != "" and Postcode.get() != "" and Mobile.get() != "" and Telephone.get() != "" and Email.get() != "":
                ...
                self.txtReceipt2.insert(END, Number_cars.get() + "\n")
                self.txtReceipt1.insert(END, 'Prime Sedan:\n')
                self.txtReceipt2.insert(END, Number_van.get() + "\n")
                self.txtReceipt1.insert(END, 'Premium Sedan:\n')
                self.txtReceipt2.insert(END, Number_threewheel.get() + "\n")
        ...
        def selectCar():
            global Item5
            if carType.get() == 1:
                self.Number_Lorry.configure(state=DISABLED)
                Number_threewheel.set("0")
                self.Number_Trucks.configure(state=DISABLED)
                Number_van.set("0")
                self.Number_threewheel.configure(state=DISABLED)
                Number_van.set("0")

                self.Number_van.configure(state=DISABLED)
                Number_van.set("0")
                self.Number_cars.configure(state=NORMAL)
                Item5 = float(8)
                Number_cars.set("Rs " + str(Item5))
            elif carType.get() == 2:
                self.Number_cars.configure(state=DISABLED)
                Number_cars.set("0")
                self.Number_van.configure(state=DISABLED)
                Number_van.set("0")
                self.Number_threewheel.configure(state=DISABLED)
                ...
                self.Number_Trucks.configure(state=DISABLED)
                Number_van.set("0")
                self.Number_Lorry.configure(state=NORMAL)
                Item5 = float(15)
                Number_threewheel.set("Rs " + str(Item5))
            else:
                self.Number_cars.configure(state=DISABLED)
                Number_cars.set("0")
                self.Number_van.configure(state=DISABLED)
        ...
        self.lblTitle = Label(Tops, font=('arial', 40, 'bold'), text="\t  SL Bloom Cab Service System ")
        self.lblTitle.grid()
        ...
        TravelFrame = LabelFrame(FrameDetails, bd=10, width=300, height=250, font=('arial', 12, 'bold'), text="Detail", relief=RIDGE)
        ...
        self.cboPickup['value'] = ('', 'Time', 'Location')
        self.cboPickup.current(0)
        ...
        self.cboDrop['value'] = ('', 'Time', 'Location')
        self.cboDrop.current(0)
        ...
        self.chkNumber_cars = Radiobutton(Book_Frame, text="Car", value=1, variable=carType,font=('arial', 14, 'bold'), command=selectCar).grid(row=0, column=0, sticky=W)
        self.txtNumber_cars = Label(Book_Frame, font=('arial', 14, 'bold'), width=7, textvariable=Number_cars, bd=5,state=DISABLED, justify=RIGHT, bg="white", relief=SUNKEN)
        self.txtNumber_cars.grid(row=0, column=1)

        self.chkNumber_van = Radiobutton(Book_Frame, text="Van", value=2, variable=carType,font=('arial', 14, 'bold'), command=selectCar).grid(row=1, column=0, sticky=W)
        self.txtNumber_van = Label(Book_Frame, font=('arial', 14, 'bold'), width=7, textvariable=Number_van, bd=5,state=DISABLED, justify=RIGHT, bg="white", relief=SUNKEN)
        self.txtNumber_van.grid(row=1, column=1)

        self.chkNumber_threewheel = Radiobutton(Book_Frame, text="Threewheel", value=3, variable=carType,font=('arial', 14, 'bold'), command=selectCar).grid(row=2, column=0 , sticky=W)
        self.txtNumber_threewheel = Label(Book_Frame, font=('arial', 14, 'bold'), width=7,textvariable=Number_threewheel, bd=5, state=DISABLED, justify=RIGHT,bg="white", relief=SUNKEN)
        self.txtNumber_threewheel.grid(row=2, column=1)

        self.chkNumber_threewheel = Radiobutton(Book_Frame, text="Truck", value=4, variable=carType,font=('arial', 14, 'bold'), command=selectCar).grid(row=3, column=0,sticky=W)
        self.txtNumber_threewheel = Label(Book_Frame, font=('arial', 14, 'bold'), width=7,textvariable=Number_Trucks, bd=5, state=DISABLED, justify=RIGHT,bg="white", relief=SUNKEN)
        self.txtNumber_threewheel.grid(row=3, column=1)

        self.chkNumber_threewheel = Radiobutton(Book_Frame, text="Lorry", value=5, variable=carType,font=('arial', 14, 'bold'), command=selectCar).grid(row=4, column=0,sticky=W)
        self.txtNumber_threewheel = Label(Book_Frame, font=('arial', 14, 'bold'), width=7,textvariable=Number_Lorry, bd=5, state=DISABLED, justify=RIGHT,bg="white", relief=SUNKEN)
        self.txtNumber_threewheel.grid(row=4, column=1)


        self.chkSingle = Radiobutton(Book_Frame, text="AC", value=1, variable=journeyType,font=('arial', 14, 'bold')).grid(row=0, column=2, sticky=W)
        self.chkReturn = Radiobutton(Book_Frame, text="Non AC", value=2, variable=journeyType,font=('arial', 14, 'bold')).grid(row=1, column=2, sticky=W)
        self.chkSpecialsNeeds = Radiobutton(Book_Frame, text="passengers", value=3, variable=journeyType,font=('arial', 14, 'bold')).grid(row=2, column=2, sticky=W)
        self.chkSpecialsNeeds = Radiobutton(Book_Frame, text="Max load and size ", value=4, variable=journeyType,font=('arial', 14, 'bold')).grid(row=3, column=2, sticky=W)
